...
const CACHE_CLEANUP_VERSION = "2026-07-fs-exit-away-v1";
...
function failPlayerApiLoad(script) {
  ...
  clearAutoplayRecovery(state.activeVideoId);
...
async function rateActiveVideo() {
  ...
  try {
    ...
    openSheet("Rating needs permission", error.message, [{ label: "Close", action: closeSheet, icon: "close" }]);
  } finally {
    finishPendingAction(actionKey);
    clearLoading();
  }
...
function render() {
  ...
  const retainedPlayer = takeReusablePlayerShell();
  if (!retainedPlayer) {
    destroyPlayer();
  }
  ...
  if (retainedPlayer) {
    ...
    hydrateRetainedPlayerShell(retainedPlayer.shell, nextPlayerShell);
    nextPlayerShell?.replaceWith(retainedPlayer.shell);
    setFullscreenButtonState(retainedPlayer.shell.classList.contains("app-fullscreen"));
    if (retainedPlayer.switchVideo) {
      switchRetainedPlayerToActiveVideo(retainedPlayer.autoplay);
    }
...
function takeReusablePlayerShell() {
  ...
  const canSwitchVisiblePlayer = state.playerReady
    && state.pendingAutoplayVideoId === state.activeVideoId
    && state.playerVideoId
    && state.playerVideoId !== state.activeVideoId
    && typeof state.player.loadVideoById === "function";
  if (!canSwitchVisiblePlayer) {
    return null;
  }

  shell.remove();
  return { shell, switchVideo: true, autoplay: true };
}

function hydrateRetainedPlayerShell(retainedShell, nextShell) {
  if (!retainedShell || !nextShell) {
    return;
  }

  const fullscreen = retainedShell.classList.contains("app-fullscreen");
  retainedShell.dataset.videoId = nextShell.dataset.videoId || state.activeVideoId;
  retainedShell.className = nextShell.className;
  retainedShell.classList.toggle("app-fullscreen", fullscreen);
...
function syncPlayerShellChild(retainedShell, nextShell, selector) {
  const retainedChild = retainedShell.querySelector(selector);
  const nextChild = nextShell.querySelector(selector);
  if (retainedChild && nextChild) {
    retainedChild.replaceWith(nextChild);
    return;
  }
  if (retainedChild && !nextChild) {
    retainedChild.remove();
    return;
  }
  if (!retainedChild && nextChild) {
...
function playerVarsFor({ autoplay = false, startSeconds = 0 } = {}) {
  return {
...
function handlePlayerReady(player, autoplayVideoId = "") {
  ...
  if (videoId && (autoplayVideoId === videoId || state.pendingAutoplayVideoId === videoId)) {
    state.pendingAutoplayVideoId = "";
    startAutoplayPlayback(player, videoId);
  }
}

function handlePlayerStateChange(player, playerState) {
  ...
  if (playerState === window.YT?.PlayerState?.PAUSED) {
    updateProgressFromPlayer({ save: true });
    stopProgressSync();
    if (autoplayRecoveryActive(videoId) && !state.autoplayHasStarted) {
      retrySoundAutoplay(player, videoId);
    }
  }
  ...
  if (playerState === window.YT?.PlayerState?.ENDED) {
    if (Date.now() < state.playerSwitchIgnoreEndedUntil) {
      return;
    }
...
function switchRetainedPlayerToActiveVideo(autoplay) {
  ...
  state.playerReady = true;
  state.playerVideoId = video.id;
  state.playerSwitchIgnoreEndedUntil = Date.now() + 1200;
  stopProgressSync();
  ...
  try {
    ...
    state.playerVideoId = previousVideoId;
    destroyPlayer();
    state.pendingAutoplayVideoId = autoplay ? video.id : "";
    mountPlayer(autoplay);
    return;
  }
...
function mountPlayer(autoplay) {
  ...
  try {
    player = new window.YT.Player(mount, {
      ...
      videoId: video.id,
      playerVars: playerVarsFor({ autoplay, startSeconds }),
      events: {
...
function settleAutoplayStart(player, videoId) {
  window.setTimeout(() => {
    if (!autoplayRecoveryActive(videoId)
      || state.player !== player
      || state.playerVideoId !== videoId
...
function forcePlayerSound(player) {
  try {
    player.unMute?.();
  } catch {}
  try {
...
function closePlayerFullscreen() {
  ...
  document.querySelectorAll(".player-fullscreen-button").forEach((button) => {
    button.classList.remove("is-pressing", "is-exiting");
  });
...
function animateFullscreenButtonAway(button) {
  if (!(button instanceof HTMLElement) || !isPlayerFullscreen()) {
    return;
  }
...
function exitPlayerFullscreenWithAnimation(button) {
  window.clearTimeout(state.fullscreenExitTimer);
  const exitButton = button || document.querySelector(".player-fullscreen-button");
  animateFullscreenButtonPress(exitButton);
  window.setTimeout(() => animateFullscreenButtonAway(exitButton), 90);
